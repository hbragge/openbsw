    ...
    ::etl::span<uint8_t> const eepBuf)
{
    ...
    if (confEntry.errorDetection)
    {
        ...
        if (!isCrcValid(::etl::span<uint8_t const>(dataPtr, usedBlockSize), eepBuf.data()))
        {
    ...
    if (confEntry.errorDetection)
    {
        ...
        calculateCrc(::etl::span<uint8_t const>(dataPtr, usedBlockSize), eepBuf.data());
    }
    ...
    ::etl::span<uint8_t> const eepBuf)
{
    ...
    if (confEntry.errorDetection)
    {
        ...
        if (!isCrcValid(
                ::etl::span<uint8_t const>(eepBuf.data() + headerSize, usedBlockSize),
                eepBuf.data()))
        {
