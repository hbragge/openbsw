...
applications using the storage API are typically located in other systems, they can get access to
...
``_storageData``. The write buffer, however, is initialized with a slice associated with only one
